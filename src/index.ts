...
export interface HaneokaHostBridge {
  ...
  writeStorage(key: string, value: unknown, signal?: AbortSignal): Promise<void>;
  deleteStorage(key: string, signal?: AbortSignal): Promise<void>;
  subscribeInput?(listener: (event: VegaInputEvent) => void, signal: AbortSignal): VegaDisposable;
...
export const HANEOKA_HOST_BRIDGE = defineVegaService<HaneokaHostBridge>("haneoka.host-bridge.v1");

export const createHaneokaPlugin = (options: HaneokaPluginOptions) => {
  const storageNamespace = sanitizeNamespace(options.storageNamespace || "haneoka");
  return defineVegaPlugin({
    manifest: {
      ...
      capabilities: ["resource", "storage", ...(options.bridge.subscribeInput ? (["input"] as const) : [])],
    },
    setup(context) {
      ...
      context.contribute("resource", {
        ...
        async load(url, signal) {
          ...
          if (!(bytes instanceof Uint8Array)) throw new TypeError("Haneoka resource bridge must return Uint8Array");
          if (bytes.byteLength > MAX_RESOURCE_BYTES) {
            throw new RangeError(`Haneoka resource exceeds ${MAX_RESOURCE_BYTES} bytes`);
          }
          return bytes;
        },
      });
      context.contribute(
        "storage",
        {
          ...
          get: (key, signal) => options.bridge.readStorage(storageKey(storageNamespace, key), signal),
          set: (key, value, signal) => options.bridge.writeStorage(storageKey(storageNamespace, key), value, signal),
          delete: (key, signal) => options.bridge.deleteStorage(storageKey(storageNamespace, key), signal),
        },
        {
          singletonPort: VEGA_STORAGE_PORT,
          ...(options.storagePriority !== undefined ? { priority: options.storagePriority } : {}),
        },
      );
      if (options.bridge.subscribeInput) {
        context.contribute(
          "input",
          {
            ...
            subscribe: (listener, signal) => options.bridge.subscribeInput!(listener, signal),
          },
          {
            singletonPort: VEGA_INPUT_PORT,
            ...(options.inputPriority !== undefined ? { priority: options.inputPriority } : {}),
          },
        );
      }
    },
...
const resourceKey = (url: URL): string => {
  ...
  if (!SAFE_RESOURCE_KEY.test(key) || key.split("/").some((part) => !part || part === "." || part === "..")) {
...
const storageKey = (namespace: string, key: string): string => {
  if (!SAFE_STORAGE_KEY.test(key) || key.includes("..")) throw new TypeError(`Unsafe Haneoka storage key: ${key}`);
...
const sanitizeNamespace = (value: string): string => {
  const namespace = value
    ...
    .slice(0, 64);
  if (!namespace) throw new TypeError("Haneoka storage namespace cannot be empty");
  return namespace;
};
...
export { haneokaStoryResourceAliases, type HaneokaStoryResourceKind } from "./resource-aliases";
export {
  hydrateStoryPayload,
...
export { createHaneokaReleaseResourceScope, type HaneokaReleaseResourceScopeOptions } from "./release-resource-scope";
export {
...
export { projectHaneokaTranscript, type HaneokaTranscriptEntry } from "./transcript.js";
