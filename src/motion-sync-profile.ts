...
export type HaneokaMotionSyncAudioScales = Readonly<Partial<Record<HaneokaVowel, number>>>;
 ...
 */
export const haneokaFallbackMotionSyncAudioScales = (entry: unknown): HaneokaMotionSyncAudioScales | undefined => {
  const source = entry && typeof entry === "object" ? (entry as Record<string, unknown>) : {};
  const characterId = Number(source.characterId);
  const characterKey = String(source.characterKey || "").padStart(3, "0");
  return characterId === 10 || characterKey === "010" ? SAKIKO_AUDIO_SCALES : undefined;
};
