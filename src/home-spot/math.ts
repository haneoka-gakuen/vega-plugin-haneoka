...
export const haneokaHomeSpotSelectionAlpha = (
...
export const haneokaHomeSpotIntroProgress = (
  value: number,
  ease: number,
): number => {
...
export const haneokaHomeSpotPointerAngles = (
  ...
  const yaw =
    pointer.x < 0
      ? -pointer.x * (Number(follow.maxLeft) || 0)
      : -pointer.x * (Number(follow.maxRight) || 0);
  const pitch =
    pointer.y < 0
      ? -pointer.y * (Number(follow.maxUp) || 0)
      : -pointer.y * (Number(follow.maxDown) || 0);
